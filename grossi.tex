...
\begin{folhadeaprovacao}
  ...
  \begin{center}
   ...
   Trabalho aprovado. \imprimirlocal, 26 de fevereiro de 2015:
...
De modo geral, as técnicas livres existentes para que essa extração de metadados seja feita são focadas em layouts pré-definidos, geralmente de conferências e/ou congressos internacionais, que possuem um padrão visual parecido, como é o caso do IEEE por exemplo, que segue de referência para diversos outros eventos tomando seu layout como base.
...
Este trabalho possui como objetivo geral identificar quais são as melhores técnicas de extração de metadados em artigos científicos com base em um conjunto de documentos para testes, dos mais diversos padrões e de diversas áreas do conhecimento.
...
Com base na diferenciação de formas de apresentação de artigos científicos este trabalho tem como Objetivos Específicos identificar pontos em que técnicas de extração de metadados existentes necessitam de adaptações por parte de seus usuários e desenvolvedores, permitindo que artigos sejam analisados de maneira diferente em virtude de especificações distintas e necessidades diferenciadas de grupos de pesquisa.

Os padrões de extração existentes no mercado são, de maneira geral, insuficientes para suprir grande parte das necessidades de eventos e/ou conferências, limitando a apenas uma pequena parcela de artigos, dentro de um \textit{layout} específico, o que acaba gerando uma desconforto e uma perda de confiabilidade das técnicas de extração de metadados existentes atualmente.

\section{Resultados Esperados}

As formas de extração de dados em artigos científicos são geralmente baseadas em \textit{layouts}, ou seja, em pequenos pedaços onde certos dados devem ser extraídos. Porém em virtude da grande diversidade de materiais produzidos, dos mais diversos padrões visuais existentes, este layout padrão geralmente utilizado em artigos científicos não se mostra eficiente na abrangência total das necessidades. 

Assim sendo, espera-se que certos artigos científicos não tenham seus metadados analisados de maneira eficaz por todas as técnicas existentes de extração de dados, uma vez que adaptações no código seriam necessárias a fim de permitir que outros padrões visuais de artigos sejam também analisados.

Deste modo espera-se que, com esta análise aprofundada das técnicas existentes, as mais eficazes na extração de metadados sejam identificadas, tendo como base de pesquisa um conjunto diversificado de artigos, de diversos grupos de pesquisa diferentes.
...
O segundo capítulo tem como base o referencial teórico feito através de uma RSL (Revisão Sistemática de Literatura), tendo como base \cite{Kitchenham}, que propõe um passo-a-passo para um revisão de literatura eficaz e atingindo os resultados desejáveis pela pesquisa.

No terceiro capítulo temos a metodologia para o desenvolvimento do trabalho, as técnicas que serão aplicadas e principalmente como serão feitas. Posteriormente, no capítulo quarto temos o desenvolvimento propriamente dito, como os testes foram realizados, os ambientes de teste, a seleção de artigos para testes e os resultados obtidos.
...
Desde modo a lista destes artigos compreende um total de 100 documentos variados, com seus metadados já extraídos manualmente e todos catalogados a fim de ter o resultado da aplicação das técnicas comparado com os resultados obtidos manualmente. Na Tabela \ref{tab:papers-list} pode-se ver o número de artigos que foram selecionados por cada área do conhecimento. A relação de todos os artigos analisados pode ser obtida no Anexo I deste documento.

...
\coorientador{Renato Souza Rocha}
\instituicao{%
...
Este trabalho possui como objetivo geral identificar quais são as melhores técnicas de extração de metadados em artigos científicos de acordo com diversos padrões existentes, tanto de formatação quanto de disposição das informações.
...
Com base na diferenciação de formas de apresentação de artigos científicos este trabalho tem como Objetivos Específicos identificar pontos em que técnicas de extração de metadados existentes permitem adaptações por parte de seus usuários, permitindo que artigos sejam analisados de maneira diferente em virtude de especificações distintas e necessidades diferenciadas de grupos de pesquisa.

Os padrões de extração existentes no mercado são, de maneira geral, insuficientes para suprir grande parte das necessidades de eventos e/ou conferências, limitando a apenas uma pequena parcela de artigos, o que acaba gerando uma desconforto e uma ineficácia das técnicas de extração de metadados existentes atualmente.
...
O projeto encontra-se ainda ativo e possui atuais contribuições de desenvolvedores ao longo do mundo. Desenvolvido na linguagem de programação \textit{Perl}, o projeto pode ser executado tanto na forma de um \textit{web service\footnote{É a disponibilização de algum serviço na Internet permitindo que outros projetos consultem suas informações e as obtém de maneira simplicada.}} quanto de maneira \textit{standalone}, com execução do código direto quando necessário.

\textit{Terminar de escrever sobre o ParsCit.}

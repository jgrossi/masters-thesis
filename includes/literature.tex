...
\emph{Regular Expressions} ou Expressões Regulares é uma técnica muito utilizada na computação para reconhecimento de padrões dentro de um conjunto de caracteres, encontrando combinações seguindo uma sequência definido.
...
Para a utilização de Expressões Regulares é necessário o fornecimento de um padrão, que será a base de busca em todo o conjunto de caracteres existente. Este padrão é representado por um conjunto de símbolos ou caracteres, que determina uma forma desejada de reconhecimento. Assim, além de utilizar-se de caracteres específicos pode-se informar números, letras, dentre outros tipos de representação, correspondendo ao que se deseja buscar dentro do texto.
...
Como exemplo, visando identificar somente os números existentes dentro da frase ``Foram encontrados 4 passageiros dentro do veículo parado na BR262.'', devemos escrever um padrão para que estes números sejam identificados. Desta forma podemos representar o que desejamos buscar da seguinte maneira: \texttt{/[0-9]+/}, onde temos:

\begin{itemize}
    \item O caractere \texttt{/} determina o começo e fim do padrão. Ele pode ser substituído por outro caractere como \texttt{\$} por exemplo, desde que seja o mesmo para identificar o início e fim do conjunto.
    \item Para representação de algarismos utilizamos a representação \texttt{[0-9]}, que significa todos os dígitos de 0 a 9, permitindo que os números sejam identificados.
    \item Como existem na frase dígitos com mais de um algarismo, como é o caso de ``262'', necessitamos complementar nosso padrão para permitir um ou mais algarismos, o que é representado pelo caractere de repetição \texttt{+}.
\end{itemize}

Para as repetições temos 6 (seis) formas de representação:
...
\begin{enumerate}
    ...
    \item Já o caractere \texttt{?} (interrogação) pode ser utilizado para representar nenhuma ou apenas uma ocorrência, ou seja, aquele padrão pode ou não existir, é opcional. No padrão \texttt{[0-9]?} estamos informando que o dígito é opcional, ou seja, ele pode ou não estar presente.
